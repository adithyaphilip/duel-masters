...
public class ImageModder {
	...
	/**
	 * Does NOT return a bitmap of specified size. It returns smallest possible bitmap larger than specified width/height
	 * @param reqWidth
	 * @param reqHeight
	 * @param path
	 * @return
	...
	{//Raw options using inJustDecodeBounds
		...
		if(reqHeight!=rwHeight || reqWidth!=rwWidth)
		{
			...
			int wRatio = Math.round(rwWidth/(float)reqWidth);//will be same as hRatio if reqWidth and height are already in aspect ratio of image
